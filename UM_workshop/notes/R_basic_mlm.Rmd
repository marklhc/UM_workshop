...
coefficient for the group mean variable becomes the _contextual_effect. 
...
```{r ran_slp}
ran_slp <- lmer(mathach ~ meanses + ses + (ses | id), data = hsball)
```
...
predict_ran_slp <- hsball %>% select(id, ses) %>% mutate(fit = predict(ran_slp))
...
\begin{align*}
  ...
  \beta_{1j} & = \gamma_{10} + \gamma_{11} \texttt{meanses}_j + 
                 \gamma_{12} \texttt{sector}_j + u_{1j}  
\end{align*}
where  
...
- $\gamma_{12}$ = cross-level interaction coefficient of the expected _slope_
...
```{r}
# Get the predicted values for plotting from `lme4`
predict_crlv_int <- hsball %>% select(id, ses, sector) %>%  # include `sector`
